...
async def get_categories_olx(message: types.Message):
    try:
        ...
        resp = requests.get('http://46.17.45.135:8080/categories', json=json)
        resp.raise_for_status()
...
# Точка входа в машину состояний команды /add_category_watch
@check_access
async def add_category_watch2(message: types.Message):
    await FSMAddCategoryWatch2.get_url.set()
    await message.answer('Отправьте ссылку для добавление в отслеживаемые категории, для отмены отправьте /cancel')


async def add_url_to_categories_watch2(message: types.Message, state: FSMContext):
    try:
        db_handler.add_category_to_watch2(message.text)
...
# Точка входа в машину состояний команды /remove_category_watch
@check_access
async def remove_category_watch2(message: types.Message):
    await FSMRemoveCategoryWatch2.get_url.set()
    await message.answer('Отправьте ссылку для удаления из отслеживаемых категорий, для отмены отправьте /cancel')


async def remove_url_from_categories_watch2(message: types.Message, state: FSMContext):
    try:
        db_handler.remove_category_from_watch2(message.text)
    except Exception:
        await message.reply('Не удалось удалить адрес из отслеживаемых')
    else:
        await message.reply('Адрес успешно удален из отслеживаемых категорий')
...
@check_access
async def get_categories_watch2(message: types.Message):
    try:
        url_list = db_handler.get_categories_from_watch2()
...
def register_admins_handlers(dp: Dispatcher):
    ...
    dp.register_message_handler(add_category_watch2, commands=['add_category_watch_2'], state=None)
